...
import { InstagramIcon, EmailIcon, LinkedInIcon, FacebookIcon } from './SocialIcons'
import {EducationIcon, ConsultingIcon, BusinessIcon} from './CategoryIcons'
import heroPortrait from '@/images/about-hero.png'
...
export function Hero({ heroTitle, heroSubtitle, heroText, heroMedia, socialLinks = [], bookCallUrl = '#', bookCallLabel = 'Book a call with me' }) {
  ...
  let safeBookCallUrl = '#'
  try {
    const parsed = new URL(bookCallUrl)
    if (['http:', 'https:'].includes(parsed.protocol)) {
      safeBookCallUrl = parsed.toString()
    }
  } catch {
    if (typeof bookCallUrl === 'string' && bookCallUrl.startsWith('/')) {
      safeBookCallUrl = bookCallUrl
    }
  }
  return (
    <section className="relative py-20 overflow-hidden lg:py-24">
      ...
      <Container className="relative z-10 grid items-center gap-16 lg:grid-cols-2 lg:gap-8">
        <div className="flex flex-col items-center max-w-2xl mx-auto lg:items-start">
          ...
          {heroSubtitle && (
            <p className="mt-3 text-xl leading-8 text-center text-slate-800 lg:text-left">{heroSubtitle}</p>
          )}
          ...
          <div className="flex flex-wrap items-center justify-center mt-10 gap-x-10 gap-y-6 lg:justify-start">
            <Button variant='secondary' href={safeBookCallUrl} className="h-11">
              {bookCallLabel}
            </Button>
            ...
            <div className="flex gap-3 sm:gap-4">
              {(socialLinks && socialLinks.length ? socialLinks : [
                ...
                const Icon = iconMap[link.icon] || iconMap[link.platform] || FacebookIcon
                ...
                return (
                  <SocialLink
                    key={`hero-social-${idx}`}
                    href={href}
                    aria-label={`Follow on ${link.platform || link.icon}`}
                    icon={Icon}
                  />
                )
              })}
